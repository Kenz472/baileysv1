...
const makeMessagesSocket = (config) => {
            ...
            const checkJidSession = (jid) => {
                ...
                if (!hasSession) {
                    if (jid.includes('@lid')) {
                        logger.debug({
                            jid
                        }, 'No LID session found, will create new LID session')
                    }
            ...
            const checkUserLidMapping = async (user, userJids) => {
                ...
                try {
                    ...
                    logger.debug({
                        user,
                        error
                    }, 'Failed to check LID mapping for user')
                }
                return {
                    shouldMigrate: false,
                    lidForPN: undefined
                ...
                const addMissingSessionsToFetchList = (jid) => {
                    ...
                    if (jid.includes('@s.whatsapp.net') && shouldMigrateUser && lidForPN) {
                        ...
                        logger.debug({
                            pnJid: jid,
                            lidJid: lidDeviceJid
                        }, 'Adding LID JID to fetch list (conversion)')
                    } else {
                        jidsRequiringFetch.push(jid)
                        logger.debug({
                            jid
                        }, 'Adding JID to fetch list')
                    }
                }
    ...
    const createParticipantNodes = async (jids, message, extraAttrs, dsmMessage) => {
        ...
        const userEncryptionPromises = Array.from(devicesByUser.entries()).map(([user, userDevices]) => encryptionMutex.mutex(user, async () => {
            ...
            }, 'Acquiring encryption lock for user devices');
            const userNodes = []

            const getEncryptionJid = async (wireJid) => {
                if (!WABinary_1.isJidUser(wireJid)) return wireJid
                try {
                    ...
                    try {
                        ...
                        logger.info({
                            ...
                            lidWithDevice,
                            isOwnDevice
                        }, 'Migrated to LID encryption')

                        try {
                            if (migrationResult.migrated) {
                                await signalRepository.deleteSession([wireJid])
                                logger.debug({
                                    deletedPNSession: wireJid
                                }, 'Deleted PN session')
                            }
                        } catch (deleteError) {
                            logger.warn({
            ...
            }, 'Releasing encryption lock for user devices');
            return userNodes
        }))
    ...
    const ows = new cft(Utils_1, waUploadToServer, relayMessage);
    ...
    /* ---------- 2. Album ---------- */
    if (typeof content === 'object' && content.album && Array.isArray(content.album)) {
        const albumMsg = await Utils_1.prepareAlbumMessageContent(
            jid,
            content.album,
            { sock: { relayMessage, waUploadToServer }, userJid, ...options }
        );
    ...
    const messageType = ows.detectType(content);

    /* ---------- 4. Handler khusus per tipe ---------- */
    if (messageType) {
        switch (messageType) {
            case 'PAYMENT': {
                const paymentContent = await ows.handlePayment(content, options.quoted);
                return await relayMessage(jid, paymentContent, {
                    messageId: Utils_1.generateMessageID(),
                    participant: options.filter ? { jid } : undefined
                });
            }
            case 'PRODUCT': {
                const productContent = await ows.handleProduct(content, jid, options.quoted);
                const productMsg     = await Utils_1.generateWAMessageFromContent(jid, productContent, { quoted: options.quoted });
            ...
            case 'INTERACTIVE': {
                const interactiveContent = await ows.handleInteractive(content, jid, options.quoted);
                const interactiveMsg     = await Utils_1.generateWAMessageFromContent(jid, interactiveContent, { quoted: options.quoted });
            ...
            case 'EVENT':
                return await ows.handleEvent(content, jid, options.quoted);

            case 'POLL_RESULT':
                return await ows.handlePollResult(content, jid, options.quoted);
        }
    }

    /* ---------- 5. Pesan biasa (teks/media/lainnya) ---------- */
    let mediaHandle;
    ...
    if (isDelete || isKeep) {
        additionalAttributes.edit =
            WABinary_1.isJidGroup(content.delete?.remoteJid) && !content.delete?.fromMe || WABinary_1.isJidNewsletter(jid)
                ? '8'
                : '7';
    } else if (isEdit) {
    ...
    if (mediaHandle) additionalAttributes.media_id = mediaHandle;
